...
public:
    ...
    void init(ISoundManager *sound, INodeDefManager *ndef)
    {
        m_sound = sound;
        m_ndef = ndef;
        m_player_step_timer = 0;
...
u32 lasttime = -1;// device->getTimer()->getTime();

extern std::wstring error_message;

Client* the_client = NULL; //y
Server *server = NULL; //y
IGameDef *gamedef = NULL;
ChatBackend* g_chat_backend = NULL;
IWritableTextureSource *tsrc = NULL;
IWritableShaderSource *shsrc = NULL;
IWritableItemDefManager *itemdef = NULL;
IWritableNodeDefManager *nodedef = NULL;
Hud* g_hud = NULL;  //y
...
GUIChatConsole *gui_chat_console = NULL; //y
gui::IGUIStaticText *guitext_chat = NULL;
gui::IGUIStaticText *guitext_profiler = NULL;
...
Camera* g_camera = NULL; //y

GameOnDemandSoundFetcher soundfetcher;
// Sound manager
ISoundManager *sound = NULL; //y
...
Sky *sky = NULL;
Inventory* g_local_inventory = NULL;
int crack_animation_length = 5;

gui::IGUIStaticText *guitext = NULL;
gui::IGUIStaticText *guitext2 = NULL;
gui::IGUIStaticText *guitext_info = NULL;
gui::IGUIStaticText *guitext_status = NULL;
// Add chat log output for errors to be shown in chat
LogOutputBuffer* g_chat_log_error_buf = NULL;


//FarMesh *farmesh = NULL;
//extern bool g_is_sneak_on;
bool cur_is_simple_singleplayer_mode = false;
extern bool simple_singleplayer_mode;
void the_game_on_connect_fail();
...
void the_game(
                  bool &kill,
                  bool random_input,
                  InputHandler *the_input,
                  IrrlichtDevice *device,
                  gui::IGUIFont* the_font,
                  std::string map_dir,
                  ...
                  bool is_simple_singleplayer_mode
                  )
{

    bool initGameSuccess = false;
    
    driver = device->getVideoDriver();
	smgr = device->getSceneManager();
    input = the_input;
    simple_singleplayer_mode = is_simple_singleplayer_mode;
    font = the_font;
    g_chat_backend = &chat_backend;
    screensize = driver->getScreenSize();
    displaycenter.X = screensize.X/2;
    displaycenter.Y =  screensize.Y/2;
...
//	v2u32 screensize = driver->getScreenSize();
	...
	tsrc = createTextureSource(device);
	
	// Create shader source
	shsrc = createShaderSource(device);
	
	// These will be filled by data received from the server
	// Create item definition manager
	itemdef = createItemDefManager();
	// Create node definition manager
	nodedef = createNodeDefManager();
...
//	Server *server = NULL;
	...
	try{
       ...
       // EventManager eventmgr;
        ...
        // Sound maker
        soundmaker.init(sound, nodedef);
        soundmaker.registerReceiver(&eventmgr);
        
        // Add chat log output for errors to be shown in chat
        static LogOutputBuffer chat_log_error_buf(LMT_ERROR);
        g_chat_log_error_buf = &chat_log_error_buf;
    ...
    do{ // Client scope (breakable do-while(0))
           ...
           // MapDrawControl draw_control;
            ...
            /*
             Create client
             */
            the_client = new Client(device, playername.c_str(), password, draw_control,
            ...
            try{
                ...
                while(device->run())
                {
                    ...
                    if(client.accessDenied()){
                        error_message = std::wstring((L"Access denied. Reason: "))
                        +  narrow_to_wide(( wide_to_narrow(client.accessDeniedReason()).c_str()));
                        errorstream<<wide_to_narrow(error_message)<<std::endl;
                        break;
                    }
                    static int connection_time_out_seconds = 60;
                    
                    if(input->wasKeyDown(EscapeKey) || time_counter > connection_time_out_seconds){
                        connect_aborted = true;
                        if (time_counter > connection_time_out_seconds)
                        {
                            error_message = narrow_to_wide("Connection time out!\n check network connection and the address:") + narrow_to_wide(address);
                            actionstream<<"Connection time out, time_counter=" << time_counter<<std::endl;
                            connection_time_out_seconds = connection_time_out_seconds + 30;
                            connection_time_out_seconds = connection_time_out_seconds > 150 ? 150 : connection_time_out_seconds;
                        }
                        else
                        {
                            actionstream<<"Connect aborted [Escape] or time out, time_counter=" << time_counter<<std::endl;
                        }
                        break;
                    }
            ...
            g_camera = new Camera(smgr, draw_control, gamedef);
            Camera& camera = *g_camera;
            if (!camera.successfullyCreated(error_message))
                return;
            
            camera_yaw = 0; // "right/left"
             ...
             */
            g_local_inventory = new Inventory(itemdef);
            Inventory& local_inventory = *g_local_inventory;
...
//            guitext_status->setVisible(false);
        core::dimension2d<u32> screensize = the_screensize;
        guitext = guienv->addStaticText(
                                        L"",
                                        core::rect<s32>(5, screensize.Height -text_height*1.02, 795, screensize.Height -text_height*1.02+text_height),
                                        false, false);
        // Second line of debug text
        guitext2 = guienv->addStaticText(
                                         L"",
        ...
        guitext_status->setVisible(false);
           ...
           // std::wstring statustext;
            ...
            gui_chat_console = new GUIChatConsole(guienv, guienv->getRootGUIElement(), -1, &chat_backend, &client);
...
//            f32 fog_range = 100*BS;
//            bool disable_camera_update = false;
              show_debug =  g_settings->getBool("show_debug");
//            bool show_profiler_graph = false;
//            u32 show_profiler = 0;
//            u32 show_profiler_max = 3;  // Number of pages
//            
...
//            float repeat_rightclick_timer = 0;
           ...
           // IntervalLimiter m_profiler_interval;
            ...
            /*
             HUD object
             */
            g_hud = new Hud(driver, guienv, font, text_height,
                    gamedef, player, &local_inventory,screensize);
            
           // bool use_weather = g_settings->getBool("weather");
            initGameSuccess = true;
            //清空下比较安全
	...
	{
		error_message = narrow_to_wide("A serialization error occurred:\n")
	...
	catch(ModError &e)
	{
		...
		error_message = narrow_to_wide(e.what()) + narrow_to_wide("\nCheck debug.txt for details.");
	}
    
    if (initGameSuccess == false)
    {
        the_game_on_connect_fail();
    }
    
}

void the_game_loop(IrrlichtDevice* device)
{
    if(device->run() == false)
        return;
...
//        error_message = L"disconnect...";
//        errorstream<< narrow_to_wide( error_message) <<std::endl;
//        error_message = L"";
        the_game_end(device);
    ...
    {
        (new GUISettings(guienv, guiroot, -1,
                             &g_menumgr, &client))->drop();
...
//    }
    ...
    else if(input->wasKeyDown(getKeySetting("keymap_toggle_time")))
    {
...
//        }
        ...
        // change time
        if (touchscreengui) touchscreengui->changeTime();
    }
...
//                device->getCursorControl()->setVisible(false);
        ...
        if(first_loop_after_window_activation){
            ...
            if (touchscreengui) {
                ...
                s32 dx = my_event_receiver.getDx();// input->getMousePos().X - displaycenter.X;
                s32 dy = my_event_receiver.getDy();//input->getMousePos().Y - displaycenter.Y;
...
//            device->getCursorControl()->setVisible(true);
        ...
        for(;;)
        {
            ...
            else if(event.type == CE_DEATHSCREEN)
            {
                ...
                new GUIDeathScreen(guienv, guiroot, -1,
                                   &g_menumgr, respawner);
                menu->drop();
                ChatBackend& chat_backend = *g_chat_backend;
    ...
    {
        InventoryList *mlist = (g_local_inventory)->getList("main");
        if(mlist != NULL)
        {
...
//                                      wtext))->drop();
    ...
    if(show_debug)
    {
        ...
        //				<<std::setprecision(3)
        //				<<", RTT = "<<client.getRTT();
        guitext->setText(char_to_wide(os.str().c_str()));
...
//        guitext2->setVisible(true);
    ...
    
    {
        ...
        if(!statustext.empty())
        {
            s32 status_y = screensize.Y * 0.75 - guitext_status->getTextHeight();
            s32 status_x = (screensize.X - guitext_status->getTextWidth()) / 2;
            core::rect<s32> rect(
                                 status_x,
                                 status_y ,
                                 status_x + guitext_status->getTextWidth(),
    ...
    ChatBackend& chat_backend = *g_chat_backend;
    LogOutputBuffer&chat_log_error_buf = *g_chat_log_error_buf;
    /*
...
extern void restore_player_name_if_needed();
void the_game_on_connect_fail()
{
    //特殊处理下某些错误信息
    std::string tmp = wide_to_char(error_message.c_str());
    const char* name_the_same = "Another client is connected with this";
    if (strncasecmp(tmp.c_str(), name_the_same, strlen(name_the_same)) == 0)
    {
        error_message = L"Player name Exist,\nChange and try it again!";
    }
    ...
    delete g_hud; g_hud = NULL;
    delete g_camera; g_camera = NULL;
   // delete g_chat_log_error_buf; g_chat_log_error_buf = NULL;
    //    delete gui_chat_console; 上面已经被drop了
    delete g_local_inventory; g_local_inventory = NULL;
//    if (farmesh)
//        delete farmesh;
//    farmesh = NULL;
...
//    touchscreengui = NULL;
    //    guienv->setSkin(GUIEngine::getSkin());
    //    GUIEngine::getSkin()->drop();
    smgr->clear();
    guienv->clear();
    //根节点要弄回来
    guiroot = guienv->addStaticText(L"",
                                    core::rect<s32>(0, 0, 10000, 10000));
    //    StateManager::get()->hardResetAndGoToScreen<HelloWorldScreen>();
    g_the_game->setUpdateGame(false);
    
    restore_player_name_if_needed();
}
void the_game_end(IrrlichtDevice* device)
{
    try{
        {
            draw_load_screen(_("Shutting down stuff..."), device, font, 0, -1, true);
        }
        g_chat_backend->addMessage(L"", L"# Disconnected.");
        g_chat_backend->addMessage(L"", L"");
	...
	{
		error_message = narrow_to_wide("A serialization error occurred:\n")
        + narrow_to_wide(e.what()) + narrow_to_wide("\n\nThe server is probably ") +
        narrow_to_wide(" running a different version of Minetest.");
