...
const int g_the_font_width = 2048;
extern int g_the_font_height;

void TextDestChat::gotText(std::wstring text)
{
    m_client->typeChatMessage(text);
}
void TextDestChat::gotText(std::map<std::string, std::string> fields)
{
...
/*
	GUIFormSpecMenu
*/
GUIFormSpecMenu* GUIFormSpecMenu::s_menu = NULL;
extern bool ingame_list_item_ui_on;
	...
	m_lock(false)
{
    ...
    GUIFormSpecMenu::s_menu = this;
    ingame_list_item_ui_on = true;
    
    if (g_the_game->isInGame())
    {
           
        item_checker.setEnable(true);
    }
}
...
GUIFormSpecMenu::~GUIFormSpecMenu()
{
    ...
    ingame_list_item_ui_on = false;
    
    if (g_the_game->isInGame())
    {
           
        item_checker.setEnable(false);
    }
}
...
void GUIFormSpecMenu::parseSize(parserData* data,std::string element) {
	...
	if (parts.size() == 2) {
        ...
        v2u32 current_screensize =  the_screensize;
		if (m_lock) {
		...
		core::rect<s32> temp = core::rect<s32>(
				data->screensize.X/2 - data->size.X/2 + offset.X,
				data->screensize.Y/2 - data->size.Y/2 + offset.Y,
				data->screensize.X/2 + data->size.X/2 + offset.X,
				data->screensize.Y/2 + data->size.Y/2 + offset.Y
		);
        float theH = temp.getHeight() * 1.0 / the_screensize.Height;
        float topLeftY = (1-theH) * 0.7 * the_screensize.Height;
        data->rect = core::rect<s32>(temp.UpperLeftCorner.X,topLeftY,
                                     temp.UpperLeftCorner.X + temp.getWidth(),
                                     topLeftY + temp.getHeight());
...
void GUIFormSpecMenu::parseCheckbox(parserData* data,std::string element) {
	...
	if ((parts.size() == 3) || (parts.size() == 4)) {
		...
		core::rect<s32> rect = core::rect<s32>(pos.X, pos.Y+((imgsize.Y/2)-g_the_font_height*1.2/2), pos.X+g_the_font_width, pos.Y+((imgsize.Y/2)+g_the_font_height*1.2/2));
...
//        float theW = pos.X+geom.X;
//        float theH_2 = theW /2.8 * stof(v_geom[1])/stof(v_geom[0]);
        float theH_2 = geom.X * stof(v_geom[1])/stof(v_geom[0]) * 0.5;
		core::rect<s32> rect = core::rect<s32>(pos.X, pos.Y-theH_2, pos.X+geom.X, pos.Y+theH_2);
...
void GUIFormSpecMenu::parseDropDown(parserData* data,std::string element) {
	...
	if (parts.size() == 5) {
		...
		core::rect<s32> rect = core::rect<s32>(pos.X, pos.Y, pos.X+width, pos.Y+g_the_font_height*1.5);
...
//    return;
	...
	if (parts.size() == 2) {
		...
		core::rect<s32> rect = core::rect<s32>(pos.X, pos.Y+((imgsize.Y/2)), pos.X+g_the_font_height, pos.Y+g_the_font_height * 1.4 * (text.length()+1));
...
void GUIFormSpecMenu::parseTabHeader(parserData* data,std::string element) {
	...
	if ((parts.size() == 4) || (parts.size() == 6)) {
		...
		v2s32 pos = padding;
		pos.X += stof(v_pos[0]) * (float)spacing.X;
		pos.Y += (stof(v_pos[1]) * (float)spacing.Y) * 1.2;
		v2s32 geom;
		geom.X = data->screensize.Y;
		geom.Y = g_the_font_height*2;
        ...
        e->setTabHeight(g_the_font_height*2);
...
const char* SELECT_ITEM_SINGLE = "Move Single Item (Double click to switch)";
const char* SELECT_ITEM_ALL = "Move All Items (Double click to switch)";
irr::core::recti the_selection_text_pos;
ItemSelectChecker::ItemSelectChecker()
{
    m_is_move_one = true;
    m_running = false;
    m_last_click_time = -DOUBLE_CLICK_CHECK_TIME_MS;
    m_status_text = L"";
}
...
//    if (g_the_game->isInGame())
//    {
//        wchar_t* text = item_checker.m_is_move_one ? wgettext(SELECT_ITEM_SINGLE) : wgettext(SELECT_ITEM_ALL);
//        int width = Environment->getSkin()->getFont()->getDimension(text).Width;
//        int theX = (the_screensize.Width-width)/2;
//        int theY = 5;
//        the_selection_text_pos = core::rect<s32>(theX,theY,
//                                               theX + width + 100,theY + g_the_font_height + 10);
//        item_checker.m_status_text = text;
//
//    }
}
...
void GUIFormSpecMenu::drawMenu()
{
	...
	m_pointer = my_event_receiver.getMousePos();// m_device->getCursorControl()->getPosition();
	...
	*/
    ...
    if (g_the_game->isInGame())
    {
     skin->getFont()->draw(item_checker.m_status_text, the_selection_text_pos, video::SColor(255,255,255,255));
    }
...
bool GUIFormSpecMenu::OnEvent(const SEvent& event)
{
			...
			&& event.MouseInput.Event != EMIE_MOUSE_MOVED)
	{
        ...
        m_pointer = my_event_receiver.getMousePos();
		updateSelectedItem();
		...
		if(updown == 0)
		{
			...
			else if(m_selected_item == NULL)
			{
				if(s_count != 0)
				{
					...
					if(button == 1)  // right
						m_selected_amount = 1;//(s_count + 1) / 2;
