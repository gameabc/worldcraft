...
- (void) touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
	...
	int idx = 0;
    ev.MultiTouchInput.PointerCount = [touches count];
	for (touch in touches)
	{
        ...
        ev.MultiTouchInput.ID[idx] = (intptr_t)touch;
...
- (void) displayCreateInWindow: (UIWindow**) window Width: (int) w Height: (int) h DriverType: (bool) type contentScale:(float)conetentScale;
...
- (void) displayCreateInWindow: (UIWindow**) window Width: (int) w Height: (int) h DriverType: (bool) type contentScale:(float)conetentScale
{
    ...
    CGRect rect = CGRectMake(0, 0, w/conetentScale, h/conetentScale);
...
namespace irr
{
    ...
    bool CIrrDeviceIPhone::createDisplay()
    {
        ...
        [((IrrIPhoneDevice*)DeviceM) displayCreateInWindow: (UIWindow**)&CreationParams.WindowId Width: CreationParams.WindowSize.Width
                                                    Height: CreationParams.WindowSize.Height DriverType: driverType
                                              contentScale: 1];
