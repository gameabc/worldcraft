...
function update_menu()
	...
	if gamedata.errormessage ~= nil then
		formspec = "size[14.4,7.5]" ..
			"field[1,2;13,2;; " ..fgettext("ERROR:") .. " " ..
			gamedata.errormessage .. 
			";]"..
			"button[5.7,6;3,0.8;btn_error_confirm;" .. fgettext("Ok") .. "]"
	else
...
function tabbuilder.dialog_create_world()
	...
	local retval = 
		"label[1,0;" .. fgettext("World name") .. "]"..
		"field[4.5,0.4;6,0.5;te_world_name;;]" ..

		"label[1,1;" .. fgettext("Seed") .. "]"..
		"field[4.5,1.4;6,0.5;te_seed;;".. current_seed .. "]" ..

		"label[1,2;" .. fgettext("Mapgen") .. "]"..
		"dropdown[4.2,2;6.3;dd_mapgen;" .. mglist .. ";" .. selindex .. "]" ..

		"label[1,3;" .. fgettext("Game") .. "]"..
		"textlist[4.2,3;5.8,2.3;games;" .. gamemgr.gamelist() ..
		";" .. menu.last_game .. ";true]" ..

		"button[4,5.8;2.8,0.8;world_create_confirm;" .. fgettext("Create") .. "]" ..
		"button[7.5,5.8;2.8,0.8;world_create_cancel;" .. fgettext("Cancel") .. "]"
...
function tabbuilder.dialog_delete_world()
	return	"label[2,2;" ..
			...
			"button[4,6;2.8,0.8;world_delete_confirm;" .. fgettext("Yes").. "]" ..
			"button[7.6,6;2.8,0.8;world_delete_cancel;" .. fgettext("No") .. "]"
end


function tabbuilder.dialog_quit_game()
	return	"label[4,2;" ..
			fgettext("Are you sure to quit the game?") .. "]"..
			"button[4,6;2.8,0.8;quit_game_confirm;" .. fgettext("Yes").. "]" ..
			"button[7.6,6;2.8,0.8;quit_game_cancel;" .. fgettext("No") .. "]"
end
--------------------------------------------------------------------------------

function tabbuilder.gettab()
	local tsize = tabbuilder.tabsizes[tabbuilder.current_tab] or {width=14.4, height=7.5}
...
function tabbuilder.handle_quit_game_buttons(fields)
	
	if fields["quit_game_confirm"] then
        print("quit the game")
        engine.close()
	else
        tabbuilder.is_dialog = false
...
function tabbuilder.handle_multiplayer_buttons(fields)
	...
	if fields["favourites"] ~= nil then
		...
		if event.typ == "CHG" and menu.favorites ~= nil and menu.favorites[event.index] ~= nil then
...
function tabbuilder.handle_helps_buttons(fields)
    if fields["feedback"] ~= nil then
        engine.feedback()
	end
    if fields["rateus"] ~= nil then
        engine.rateus()
	end
end
--------------------------------------------------------------------------------
function tabbuilder.handle_settings_buttons(fields)
    ...
    if fields["cb_inventory_2d"] then
		engine.setting_set("using_2d_inventory", fields["cb_inventory_2d"])
	end
    if fields["cb_show_debug"] then
		engine.setting_set("show_debug", fields["cb_show_debug"])
	end
...
function tabbuilder.handle_singleplayer_buttons(fields)
    if gamedata.local_server_port ~= nil then
        print("11"..gamedata.local_server_port)
    end
    if fields["local_server_port"] ~= nil then

		gamedata.local_server_port = fields["local_server_port"]
		engine.setting_set("local_server_port", fields["local_server_port"])
        print("22"..gamedata.local_server_port)
	end
	...
	if fields["play"] ~= nil or
		...
		if selected > 0 then
            ...
            gamedata.errormessage = fgettext("ServerError: Select or new a game to play!")
		end
	end
...
function tabbuilder.tab_multiplayer()
	...
	local retval =
		"vertlabel[0,-0.25;".. fgettext(" MULTIPLAYER") .. "]" ..
		"label[1,-0.4;".. fgettext("Favorites:") .. "]"..
		"label[1,5.5;".. fgettext("Address/Port") .. "]"..
		"label[9,3.3;".. fgettext("Name/Password") .. "]" ..
		"field[1.25,6.5;5.5,0.8;te_address;;" ..engine.setting_get("address") .."]" ..
		"field[6.75,6.5;2.25,0.8;te_port;;" ..engine.setting_get("port") .."]" ..
		"checkbox[1,4.5;cb_public_serverlist;".. fgettext("Public Serverlist") .. ";" ..
		dump(engine.setting_getbool("public_serverlist")) .. "]"
		
	if not engine.setting_getbool("public_serverlist") then
		retval = retval .. 
		"button[5.8,4.7;2.75,0.8;btn_delete_favorite;".. fgettext("Delete") .. "]"
	end
	
	retval = retval ..
		"button[9,6.2;3.5,1;btn_mp_connect;".. fgettext("Connect") .. "]" ..
		"field[9.3,4.3;2.5,0.8;te_name;;" ..engine.setting_get("name") .."]" ..
		"pwdfield[9.3,5;2.5,0.8;te_pwd;]" ..
		"textarea[9.3,0.25;5,4;;"
	...
	retval = retval .. 
		";]" ..
		"textlist[1,0.35;7.5,3.95;favourites;"
...
function tabbuilder.tab_settings()
	return	"vertlabel[0,0;" .. fgettext("  SETTINGS") .. "]" ..
            "label[1,5.8;".. fgettext("Disable some effects can get better performance") .. "]" ..
            "label[1,6.5;".. fgettext("If the ingame 3D inventory don't work, try to use the 2D Inventory") .. "]" ..
			"checkbox[1,0.75;cb_fancy_trees;".. fgettext("Fancy trees") .. ";" 
					.. dump(engine.setting_getbool("new_style_leaves")) .. "]"..
			"checkbox[1,1.5;cb_smooth_lighting;".. fgettext("Smooth Lighting")
					.. ";".. dump(engine.setting_getbool("smooth_lighting")) .. "]"..
			"checkbox[6,0.75;cb_3d_clouds;".. fgettext("3D Clouds") .. ";"
					.. dump(engine.setting_getbool("enable_3d_clouds")) .. "]"..
			"checkbox[6,1.5;cb_opaque_water;".. fgettext("Opaque Water") .. ";"
					.. dump(engine.setting_getbool("opaque_water")) .. "]" ..
            "checkbox[9.5,1.5;cb_show_debug;".. fgettext("FPS") .. ";"
					.. dump(engine.setting_getbool("show_debug")) .. "]" ..
            "checkbox[9.5,0.75;cb_inventory_2d;".. fgettext("2D Inventory") .. ";"
					.. dump(engine.setting_getbool("using_2d_inventory")) .. "]"
...
function tabbuilder.tab_singleplayer()
	...
	return	"button[5.5,4.6;3.5,0.8;world_delete;".. fgettext("Delete") .. "]" ..
			"button[9.6,4.6;3.5,0.8;world_create;".. fgettext("New") .. "]" ..
			"button[9.6,6;3.5,1;play;".. fgettext("Play") .. "]" ..
			"label[5.5,-0.4;".. fgettext("Select World:") .. "]"..
			"vertlabel[0,-0.25;".. fgettext("SINGLEPLAYER") .. "]" ..
			"checkbox[1,0.25;cb_creative_mode;".. fgettext("Creative Mode") .. ";" ..
			dump(engine.setting_getbool("creative_mode")) .. "]"..
			"checkbox[1,1;cb_enable_damage;".. fgettext("Enable Damage") .. ";" ..
			dump(engine.setting_getbool("enable_damage")) .. "]"..
            "label[1,1.75;".. fgettext("Local IP/Port") .. "]"..
            "label[1,2.4;".. engine.setting_get("local_server_ip") .. "]"..
            "field[1.2,3.5;2.25,0.8;local_server_port;;" ..engine.setting_get("local_server_port") .."]" ..
			"textlist[5.5,0.45;7.5,3.9;sp_worlds;" ..
			menu.render_world_list() ..
			";" .. index .. "]" ..
			menubar.formspec
end
...
function tabbuilder.tab_credits()
	...
	return	"vertlabel[0,-0.5;   CREDITS]" ..
			...
			"textlist[4,-0.25;9.5,5.8;list_credits;" ..
...
----------------------
function tabbuilder.tab_helps()
	local logofile = menu.defaulttexturedir .. "logo.png"
	return	"vertlabel[0,-0.5;   HELP]" ..
            "textlist[1,0.05;12.5,6;list_helps;" ..
            "#FFFF00" .. fgettext("Multiplayer For WIFI") .."," ..
            "Server: Start a game from the 'Singleplayer' tab," ..
            "           Share 'Local IP/Port' to your friends," ..
            "Client: Login to the server from the 'Multiplayer' tab," ..
            "           Using server's IP and port," ..
            "," ..
			"#FFFF00" .. fgettext("Basic Movement") .."," ..
            "Use the touchpad at the bottom left side of the screen to move.," ..
            "Touch the screen besides the buttons to look around.," ..
            "Tap the center button of the touch pad to jump/fly.," ..
            "," ..
			"#FFFF00" .. fgettext("In Game Buttons") .. "," ..
            "Right side buttons can toggle fly/sneak/noclip mode.," ..
            "(note: ask/grant needed privileges to do it)," ..
            "Inventory button is on the left.," ..
            "Tap the chat button to send messages/commands.,"..
            "," ..
			"#FFFF00" .. fgettext("Basic Action") .. "," ..
            "Click the center of the screen to place a block.,"..
            "Hold your finger to dig the marked block.," ..
			";0;true]" 
end
--------------------------------------------------------------------------------
function tabbuilder.init()
	tabbuilder.tabfuncs = {
        ...
        helps         = tabbuilder.tab_helps,
		dialog_create_world = tabbuilder.dialog_create_world,
		dialog_delete_world = tabbuilder.dialog_delete_world,
        dialog_quit_game    = tabbuilder.dialog_quit_game
	}

	tabbuilder.tabsizes = {
		dialog_create_world = {width=14.4, height=7.5},
		dialog_delete_world = {width=14.4, height=7.5},
        dialog_quit_game    = {width=14.4, height=7.5}
	}
    ...
    table.insert(tabbuilder.current_buttons,{name="helps", caption=fgettext("Help")})
...
engine.button_handler = function(fields)
    ...
    if tabbuilder.current_tab == "dialog_quit_game" then
		tabbuilder.handle_quit_game_buttons(fields)
	end
    ...
    if tabbuilder.current_tab == "helps" then
        tabbuilder.handle_helps_buttons(fields)
    end
...
engine.event_handler = function(event)
	if event == "MenuQuit" then
		if tabbuilder.is_dialog then
            ...
            tabbuilder.current_tab = "dialog_quit_game"
            tabbuilder.is_dialog = true
            tabbuilder.show_buttons = false
            update_menu()
			--engine.close()
